...
import { Calendar, MapPin, Share2, ThumbsUp, ThumbsDown, CheckCircle } from 'lucide-react'

export default function EventPage() {
  const params = useParams()
  const [event, setEvent] = useState<any>(null)
  const [status, setStatus] = useState<string | null>(null) // 'going', 'not_going'
            ...
            <div className="md:col-span-2 space-y-8">
                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
                    ...
                    <p className="text-slate-600 leading-relaxed text-lg">
                        Join us for an unforgettable night at {event.venue_name}. {event.description || 'Experience the best music, food, and vibes in Dimapur.'}
                    </p>
                    <div className="mt-6 flex gap-4 text-sm font-bold text-slate-500">
                        ...
                        <div className="flex items-center gap-2 bg-slate-50 px-4 py-2 rounded-lg">
                            <MapPin className="w-4 h-4"/> Dimapur
                        </div>
                    </div>
                </div>
            </div>
